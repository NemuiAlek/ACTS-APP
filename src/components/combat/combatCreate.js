...
*/
  const createBtn = (event, action) => {
    ...
    axios
      ...
      .then((response) => {
        updateMode
          ? getSavedCombats()
          : navigate("/combattrack/" + response.data.id);
        setCombat({ ...combat, id: 0, name: "", desc: "" });
        setUpdateMode(false);
      })
      .catch((err) => {
        console.log(err);
  ...
  const getSavedCombats = () => {
    axios
      .get(
        "https://acts-api-production.up.railway.app/combat/" + (theUser.id ? theUser.id : "Guest"),
        {}
      )
      .then((response) => {
  ...
  const updateField = (id, name, desc) => {
    setCombat({ ...combat, id: id, name: name, desc: desc });
  };
    ...
    <div id="createCombat">
      <div id="combatInfo">
        <div id="topInfo">
          <h5>
            {theUser
              ? "Logged in as " +
                theUser.userName +
                " (Combat will be saved after each round)"
              : "GUEST MODE: (Combat will be lost after 24hr. Please sign up to save combat permanently.)"}
          </h5>
          {theUser && !updateMode && (
            <Button
              variant="secondary"
          ...
          {theUser && updateMode && (
            <label>
              Update mode activated. Click on any combate from the list to
              update then click save.
            </label>
          )}
        </div>
        ...
        <div className="inputGrouping top">
          ...
          <Button
            ...
            size="lg"
            onClick={(event) =>
              updateMode ? createBtn(event, "update") : createBtn(event, "new")
            }
          >
            Save
          </Button>
      ...
      <div id="savedCombats">
        <Table striped hover size="sm" className="monsterList">
          ...
          <tbody>
            {Array.isArray(savedCombats) &&
              savedCombats.map((combat) => (
                <tr
                  key={combat.id}
                  onClick={() => {
                    if (!updateMode) {
                      navigate("/combattrack/" + combat.id);
                    } else {
                      updateField(combat.id, combat.name, combat.desc);
                    }
                  }}
                >
                  <td>{combat.name}</td>
