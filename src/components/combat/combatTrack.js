...
*/
  ...
  const addMonster = (event, action) => {
    ...
    if (combatDetail.initiative === 0) {
      ...
      alert(
        `Well, I guess we all have had our fair share of embarassing moments...`
      );
      alert(
        `Maybe it's not fair to throw away the dice just because of one bad roll you know?`
      );
      alert(
        `Yea! It was probably even a good laugh for you as a DM when they or you rolled it too`
      );
      alert(
        `Ahh... fun times... I havent slept in 24 hours developing this web app...`
      );
      alert(
        `But you know, it'll all be worth it once it's done! I'm so clooose`
      );
      alert(
        `OH RIGHT! Sorry about that, let me create that... uhh.. "zero initiative" creature for you.`
      );
      alert(
        `oh! and thank you for using my website! it means a lot! Hope your session goes great!`
      );
    }
    ...
    if (
      combatDetail.maximumHP &&
      combatDetail.initiative &&
      combatDetail.multiplier === "" &&
  ...
  const updateOptions = () => {
    ...
    if (dictionary === "standard") {
      monsters.map((eachMonster) =>
        setOptions((x) => [
          ...x,
          { value: eachMonster.slug, label: eachMonster.name },
        ])
      );
      ...
      monsters.map((eachMonster) =>
        setOptions((x) => [
          ...x,
          { value: eachMonster.id, label: eachMonster.name },
        ])
      );
    }
  };
...
*/
  ...
  useEffect(() => {
    dictionary.toLocaleLowerCase() === "standard"
      ? getMonstersStandard()
      : getMonstersCustom();
  }, [dictionary]);
  ...
  return (
    <div id="createDetail">
      <div id="combatInfo">
        <div id="combatStats">
          ...
          {deleteMode && (
            <Button
              ...
              className="profileEditOptions"
              size="sm"
              onClick={(event) => deleteCombat(event, params.id)}
            >
              Confirm Delete (THIS ACTION IS PERMANENT)
            </Button>
          )}
        </div>
        <h1>Add Monsters</h1>
        ...
        <div className="inputGrouping">
          ...
          <Button
            ...
            className="profileEditOptions"
            size="lg"
            onClick={(event) => addMonster(event, "new")}
          >
            Add
          </Button>
        </div>
              ...
              <div className="top-stats">
                ...
                <div className="property-line last">
                  ...
                  {Array.isArray(monster.speed) &&
                    isNaN(combatDetail.monsterid) &&
                    dictionary === "standard" &&
                    monster.speed &&
                    Object.entries(monster.speed).map((array) => (
                      <p key={array[0]}>
                        {array[0] === "walk" ? "" : array[0]} {array[1]}ft.,{" "}
                      </p>
                    ))}
              ...
              {monster.special_abilities && monster.special_abilities && (
                <div>
                  {monster.special_abilities.map((array) => (
                    <div
                      key={array.name + "specialTopDiv"}
                      className="property-block"
                    >
                      <h4 key={array.name + "specialTop"}>
                        {array.name}{" "}
                        {array.usage ? "(" + array.usage.times + "/Day)" : ""}.{" "}
                      </h4>
      ...
      <div className="monsterCard">
        {combatMonsters &&
          combatMonsters.map((eachMonster) => (
            <div
              className="stat-block wide"
              onClick={(event) =>
                setMonsterIDScroll(event, eachMonster.monsterID)
              }
            >
              ...
              <div className="section-left">
                <div className="creature-heading">
                  ...
                  <h2>
                    {eachMonster.monsterSize} {eachMonster.monsterType},{" "}
                    {eachMonster.monsterAlignment}
                  </h2>
                </div>
                <svg height="5" width="100%" className="tapered-rule">
                  <polyline points="0,0 400,2.5 0,5"></polyline>
                ...
                <div className="top-stats">
                  <div className="property-line first">
                    <h4>Armor Class </h4>
                    <p>
                      {eachMonster.monsterAC} {eachMonster.monsterArmor}
                    </p>
                  </div>
                  <div className="property-line">
                    <h4>Hit Points </h4>
              ...
              <Button
                ...
                className="profileEditOptions"
                size="sm"
                onClick={(event) => deleteDetail(event, eachMonster.id)}
              >
                Delete
              </Button>
            </div>
